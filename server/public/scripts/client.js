...
function clickHandler() {
  ...
  $("#noteList").on("click", ".delete-button", deletePrep);
...
function appendTasks(getArray) {
  ...
  for (const task of getArray) {
    ...
    task.is_complete
      ? (completeClass =
          "table-success text-decoration-line-through color-green")
      : (completeClass = "");
    ...
    $("#noteList").append(`
      ...
      task.is_complete
                ...
                <td class="text-wrap">${task.note}</td>
                <td class="">${
              ...
              </svg></button><button class="edit-button btn-sm btn btn-outline-secondary mx-1"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gear" viewBox="0 0 16 16">
              ...
              </svg></button><button class="delete-button btn-sm btn btn-outline-danger m-0" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
...
function addTask() {
  ...
  // Validating data
  const inputData = $("#inputNote").val();
  if (!inputData || inputData === " ") return alert("Cannot add blank tasks!");
